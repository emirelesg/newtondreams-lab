/*
 * Fire shader modified to use javascript ES6.
 * Based on three-particle-fire by @yomotsu (c) 2017.
...
class Geometry extends BufferGeometry {
  constructor(radius, height, particleCount) {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      position[i * 3 + 0] = Math.cos(angle) * r * 3;
    ...
    this.setAttribute('randam', new BufferAttribute(randam, 1));
...
class Material extends ShaderMaterial {
  constructor(parameters) {
    super({
      ...
      vertexShader: [
        ...
        'vOpacity = min( influence * 4.0, 1.0 ) * progressNeg * opacity;',
        'vSprite = sprite;',
        '}'
      ].join('\n'),
      fragmentShader: [
  ...
  setPerspective(fov, height) {
    this.uniforms.heightOfNearPlane.value = Math.abs(
      height / (2 * Math.tan(fov * 0.5 * DEG_TO_RAD))
    );
  }
}
